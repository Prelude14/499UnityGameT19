...
//Our playermanager object is going to be spawned unique to each client, but it have to handle 
//all of the networking as a client or a server
public class playerManager : NetworkBehaviour
{
    ...
    //game objects to be spawned and synced on server or clients
    ...
    //List<GameObject> combinedDeck = new List<GameObject>(); //actual game object version of the list
...
=======
    ...
    {
        //server should automatically tell the clients that the sync var clientDeck has updated at the end of the current frame
        if (isServer)
        {
            clientDecks += clientColour;
            Debug.Log("actual serverside: " + clientDecks);
        }
        else
        {
            //Debug.Log("client made it to servercheck 2nd else, and will call command to get server to update clientdecks now.");
            CmdGetPlayerColours(clientColour);
        }//each client should have its own private playerDeck colour, and the server will mix them
...
>>>>>>> Stashed changes
    ...
    [Command]
    public void CmdCreateDeck() //want to have method for checking if enough players have joined
    {
        //once 2 clients have connected and they have a valid combo, the server can populate the combinedDeck using the proper combo
        //WANT SERVER TO CREATE DECK, AND THEN DEAL CARDS TO CLIENT USING RPC
...
=======
        ...
        while (validDeckShuffled)
        {
            //if server hasn't shuffled a valid deck yet, stay inside while loop, keep checking for enough players, and create deck if there is enough -- this will result in exit of while loop
            CmdCreateDeck();
        }
        */
>>>>>>> Stashed changes
